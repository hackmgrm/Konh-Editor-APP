...
| 内置主题 | 原版主题库 | 扩展为 30 套，并加入空核域界品牌版式 |
...
- 30 套内置主题（浅色 / 深色纸底）、三档排版密度；每篇文章可独立选择主题
- 空核域界品牌主题支持 front-matter 自动头图、导语、标签和签名卡
- OpenAI 兼容 API Agent，可直接在当前工作区读写文章，不依赖本地终端
- 深言创作：按主题、领域、结构、语气和篇幅一键生成或改写公众号文章
- 去 AI 痕迹：选区或全文分三档处理，查看差异后确认替换
- Cloudinary 图床：扫描当前文章本地图、去重上传并可选替换为永久 HTTPS 链接
...
- 专注模式提供稳定的单栏 Markdown 写作并隐藏文章元数据；打字机模式让当前编辑行保持在视野中央
- 编辑区可开启即时排版，在非当前行隐藏标题、强调、删除线和行内代码的 Markdown 标记
- 文件名自动跟随文章标题：空核域界主题读取 Front Matter `title`，其他主题读取首个 H1
...
“深言创作”的分类与文章结构工作流改编自 [honwhy/DeepTalk](https://github.com/honwhy/DeepTalk)（ISC）。
